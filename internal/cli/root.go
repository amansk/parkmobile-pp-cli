// Package cli is the cobra command tree for parkmobile-pp-cli.
package cli
...
import (
	...
	"github.com/amansk/parkmobile-pp-cli/internal/output"
...
func newRoot(opt *Options) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:           "parkmobile-pp-cli",
		Short:         "Agent-native ParkMobile consumer zone parking CLI",
		Long:          "List zones, sessions, and (hard-gated) start/extend/stop zone parking via ParkMobile Phonixx consumer APIs. Never prints secrets.",
		Version:       version,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	cmd.AddCommand(newSessionCmd(opt))
	return cmd
}
