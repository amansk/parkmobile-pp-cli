...
func newAuthCmd(opt *Options) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage ParkMobile session authentication",
	}
...
func newAuthLoginCmd(opt *Options) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "login",
		Short: "Import ParkMobile session cookies",
		Long:  "Sign in at parkmobile.io (Zone Parking) in Chrome, then run with --chrome, or export Playwright storage-state JSON / raw Cookie header via --cookies-file. Cookie files are stored mode 0600; secrets are never printed.",
	...
	cmd.Flags().BoolVar(&useChrome, "chrome", false, "Import cookies from local Chrome (sign in at parkmobile.io first)")
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if sess == nil {
				...
				out["hint"] = "run: parkmobile-pp-cli auth login --chrome"
