...
func ImportChromeCookies() (*Session, error) {
	...
	if _, err := os.Stat(cookiesDB); err != nil {
		return nil, fmt.Errorf("chrome cookies database not found at %s: sign in at parkmobile.io first", cookiesDB)
	}
...
out = {}
for c in jar:
    if "parkmobile" in c.domain.lower():
        out[c.name] = c.value
...
`
	...
	if len(cookies) == 0 {
		return nil, fmt.Errorf("no parkmobile cookies in chrome; sign in at https://parkmobile.io/login (Zone Parking) first")
	}
	return &Session{
		Cookies:   cookies,
		Source:    "chrome",
...
func importViaSQLiteCopy(cookiesDB string) (*Session, error) {
	if _, err := exec.LookPath("sqlite3"); err != nil {
		return nil, fmt.Errorf("chrome import needs python3+browser_cookie3 or sqlite3; sign in at parkmobile.io and use auth login --cookies-file")
	}
	...
	defer os.Remove(tmp)
	query := `SELECT name, value FROM cookies WHERE host_key LIKE '%parkmobile%'`
