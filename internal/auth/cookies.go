...
func isParkMobileDomain(domain string) bool {
	d := strings.ToLower(domain)
	return strings.Contains(d, "parkmobile") || strings.Contains(d, "phonixx")
}
...
func ParseCookiesFile(data []byte, source string) (*Session, error) {
	...
	if strings.HasPrefix(trimmed, "{") {
		...
		for _, c := range state.Cookies {
			if !isParkMobileDomain(c.Domain) {
				continue
			}
			if c.Name != "" && c.Value != "" {
		...
		if len(cookies) == 0 {
			return nil, fmt.Errorf("no parkmobile cookies found in storage state")
		}
		return &Session{
			Cookies:   cookies,
			Source:    source,
